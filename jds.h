struct UPP { /* structure contains all the informations about data processing, PP has 16 bytes */
   ...
   unsigned int fft_size; /* used for data trasferering */
   unsigned int junk[10];
   /* unsigned int junk[26]; use this value if you want to read old files, before August 2008 */
};
...
struct UDSPP { /* structure contains extended informations about about data processing and data acquisition mode, DSPP has 112 bytes */
   ...
   int Smd; /* spectrum stream transfer to DSP (0 - streams A & B, 1 - stream A & A*B, 2 - stream B & A*B, 3 - A+B & A*B) */
   ...
   char DSPv[32];
   int junk[66];
   /* int junk[50]; use this value if you want to read old files, before August 2008 */
};

struct syst { /* this structure contains systemtime, it must be short int because IDL is using 2-byte integers to store the information */
   short int yr; /* year */
   short int mn; /* month */
...
struct FHEADER {      /* file header, 1024 bytes size with following fields */
