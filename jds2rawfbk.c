// ADD WRITING OF SMAPLES TO FILE IN unisgned int FORMAT FOR OTHER PROGRAMS
...
int main(int argc, char *argv[]) {
   ...
   telescopeID = 10; /* ID of the telescope, using TEMPO or PRESTO convention */
   ...
   if (argc < 2 || (strcmp(argv[1], "-h") == 0)) { /* in case there is a -h switch */
      ...
      printf("\nUsage: jds2rawfbk [parameters] <filename>\n");
      printf("\nAvailable parameters are:\n");
      printf("-psrname     Name of the source to be written in .fbk file (default: %s)\n", psrName);
      ...
      printf("-tel         Telescope SIGPROC identifier (NRT: 3, UTR2: 10, etc.; default: %d)\n", telescopeID);
      ...
      printf("-o           Give new name of output file (default: add .fbk to original filename trunk)\n");
      ...
      for (argument = 1; argument < argc - 1; argument++) {
         if (strcmp(argv[argument], "-psrname") == 0) {
            ...
            if (freqRanges != 2 || freqRanges > 2) {
               fprintf(stderr, "Error parsing -freq option.\n");
               return -1;
            }
            if (freqLow >= freqHigh || freqHigh <= freqLow || freqLow <= 0.0 || freqHigh <= 0.0) {
               fprintf(stderr, "Error passing frequency ranges.\n");
               return -1;
            }
            Frequency = 1;
            argument++;
            ...
            if (clipInput != 2 || clipInput > 2) {
               fprintf(stderr, "Error parsing -clip option.\n");
               return -1;
            }
            ClipSpectrum = 1;
            argument++;
            ...
            if (streamOperation < 0 || streamOperation > 4) {
               fprintf(stderr, "Error parsing -stream option.\n");
               return -1;
            }
            argument++;
   ...
   startMJD = gregjd(jy, jm, jd, (double) jhr, (double) jmn, jsc) - 2400000.5; /* convert the date to JD and add 2400000.5 to get MJD */
   ...
   if (OutFilename == 1) {
      if ((fbkFilename = malloc(strlen(outFilename) + 1)) == NULL) {
         ...
         sprintf(fbkFilename, "%s", outFilename);
      }
   } else {
      fbkFilename = removeExtension(jdsFilename, '.', '/'); /* remove extension using external function */
      if (HeaderLess == 0) {
         strcat(fbkFilename, ".fbk"); /* add new extension */
      } else if (HeaderLess == 1) {
         strcat(fbkFilename, ".dat"); /* add new extension */
      }
   }
   printf("%s> Creating file %s\n", argv[0], fbkFilename);
   if ((FILTERBANK_OUT = fopen(fbkFilename, "w")) == NULL) {
      fprintf(stderr, "%s> Unable to open file %s\n", argv[0], fbkFilename);
      return -1;
   }
   /* SENDING HEADER INFORMATION TO .fbk FILE HEADER */
   if (HeaderLess == 0) {
      send_string("HEADER_START");
      send_string("rawdatafile"); /* setting... */
      send_string(fbkFilename); /* ... the name of the original data file */
